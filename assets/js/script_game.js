...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let fabMode = 'down'; // 'down' = ir p/ pistas | 'up' = voltar p/ jogo

    function setFabMode(mode) {
        fabMode = mode;

        if (mode === 'up') {
            fabIcon.className = 'bi bi-arrow-up';
            fabText.textContent = 'Jogo';
            fab.setAttribute('aria-label', 'Voltar para o jogo');
        } else {
            fabIcon.className = 'bi bi-journal-text';
            fabText.textContent = 'Pistas';
            fab.setAttribute('aria-label', 'Ir para pistas');
        }
    }

    if (fab && gameSection && cluesSection) {
        // Clique do botão
        ...
        // Observa quando as pistas entram na tela -> muda para "subir"
        const obsClues = new IntersectionObserver(([entry]) => {
            if (entry.isIntersecting) setFabMode('up');
        }, { threshold: 0.25 });

        // Observa quando o jogo entra na tela -> muda para "descer"
        const obsGame = new IntersectionObserver(([entry]) => {
            if (entry.isIntersecting) setFabMode('down');
        }, { threshold: 0.25 });

        obsClues.observe(cluesSection);
        obsGame.observe(gameSection);
